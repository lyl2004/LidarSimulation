...
为避免 `a/c` 与 `c/a`、几何半轴与等体积半径长期混淆，项目层面建议以后统一使用以下命名：
...
- `pytmatrix`：`radius = r_eqv`，`axis_ratio = a_eq / c_pol`，`radius_type = RADIUS_EQUAL_VOLUME`

### 14.1 单粒子对照参考数值（本地 TransitionMatrices）

示例半径点 `r = 0.8 / 1.1 / 1.5 μm`：

| r (μm) | sigma_ext | sigma_sca | sigma_back | phase_back |
| ---: | ---: | ---: | ---: | ---: |
| 0.8 | 1.149650e-11 | 1.096084e-11 | 2.230859e-12 | 0.203530 |
| 1.1 | 1.773513e-11 | 1.635833e-11 | 7.155564e-12 | 0.437426 |
| 1.5 | 2.125603e-11 | 1.785526e-11 | 1.252897e-11 | 0.701696 |

分布级 coarse 对照（本地参考：`alpha=3.809514e-03`、`beta=1.077492e-03`）：
...
本地结果在多数半径上更接近 `semantic_prolate`，但在某些半径上又更像 `current_oblate`，即并不稳定贴近某一种 `pytmatrix` 几何解释 —— 这与第 5~9 节"剩余差异源于粒子级后向散射核"的结论一致。
