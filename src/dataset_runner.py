...
def build_quality(
    ...
    linear_depol = np.asarray(observation.get("linear_depol_ratio", []), dtype=np.float64)
    valid = counts > 0.0
    ...
    return {
        ...
        "median_linear_depol_ratio": float(np.median(linear_depol[usable_mask])) if linear_depol.shape == counts.shape and np.any(usable_mask) else None,
        "max_linear_depol_ratio": float(np.max(linear_depol[usable_mask])) if linear_depol.shape == counts.shape and np.any(usable_mask) else None,
        "polarization_fields": {
            "echo_parallel_power": "parallel linear-polarized echo power, computed as 0.5 * (echo_I + echo_Q)",
            "echo_perpendicular_power": "perpendicular linear-polarized echo power, computed as 0.5 * (echo_I - echo_Q)",
            "linear_depol_ratio": "linear depolarization ratio, echo_perpendicular_power / echo_parallel_power",
            "echo_depol": "legacy polarization-loss diagnostic, 1 - sqrt(Q^2 + U^2 + V^2) / I",
        },
