...
_DIAG_SESSION = get_or_create_session("iitm_worker")
_DIAG_LOGGER = new_component_logger(_DIAG_SESSION, "iitm_worker")
...
def _diag_event(stage: str, *, status: str = "ok", elapsed_ms: float | None = None, payload: dict | None = None) -> None:
    ...
    _DIAG_SESSION.write_component_event("worker_timeline.jsonl", stage, status=status, elapsed_ms=elapsed_ms, payload=payload)
...
def resolve_cpu_limit(cpu_limit: str) -> str:
    if cpu_limit == "auto":
        return "auto"
    return str(max(1, int(cpu_limit)))
...
def start_julia_server(cpu_limit: str = "auto") -> subprocess.Popen:
    """
    启动 Julia HTTP 服务进程。
    ...
    resolved_cpu_limit = resolve_cpu_limit(cpu_limit)
    env = os.environ.copy()
    env["JULIA_NUM_THREADS"] = resolved_cpu_limit

    cmd = [
        "pixi", "run", "-e", "julia",
        "julia",
        f"--threads={resolved_cpu_limit}",
...
def ensure_julia_server(cpu_limit: str = "auto") -> bool:
    """
    ...
    if info:
        ...
        _diag_event("julia_server_reused", payload={"threads": running_threads, "cpu_limit": cpu_limit})
        return True

    print(f">> [Worker] Julia 服务未就绪，正在启动（超时 {SERVER_START_TIMEOUT}s）...",
          flush=True)
    _diag_event("julia_server_starting", status="begin", payload={"cpu_limit": cpu_limit, "timeout_s": SERVER_START_TIMEOUT})
    ...
    while time.time() < deadline:
        if is_server_alive():
            ready_elapsed = time.time() - start_wait
            print(f"\n>> [Worker] Julia 服务就绪（用时 {time.time()-deadline+SERVER_START_TIMEOUT:.1f}s）",
                  flush=True)
            _diag_event("julia_server_ready", elapsed_ms=ready_elapsed * 1000.0, payload={"cpu_limit": cpu_limit})
            return True
        if _julia_proc.poll() is not None:
            print(f"\n>> [Worker] Julia 进程意外退出 (code={_julia_proc.returncode})",
                  flush=True)
            _diag_event("julia_server_ready", status="error", elapsed_ms=(time.time() - start_wait) * 1000.0, payload={"returncode": _julia_proc.returncode})
            return False
        time.sleep(2)
        dot_count += 1
        if dot_count % 5 == 0:
    ...
    _diag_event("julia_server_ready", status="error", elapsed_ms=(time.time() - start_wait) * 1000.0, payload={"reason": "timeout"})
    return False
...
def main():
    ...
    parser.add_argument("--cpu_limit",    default="auto")
    args = parser.parse_args()

    result_payload = {"status": "failed", "metrics": {}, "artifacts": []}
    worker_t0 = time.time()
    _diag_event("worker_start", status="begin", payload={"project_name": args.project_name, "cpu_limit": args.cpu_limit})
    ...
    try:
        ...
        _diag_event("julia_stream_connected", status="begin", payload={"endpoint": SIMULATE_ENDPOINT})
        t0 = time.time()
        ...
        dt = time.time() - t0
        _diag_event("julia_request_finished", elapsed_ms=dt * 1000.0, payload={"project_name": args.project_name})
        ...
        if julia_response.get("status") == "success":
            ...
            print(f">> [Worker] 仿真成功，耗时 {dt:.2f}s，R_back={metrics.get('R_back',0):.6f}",
                  flush=True)
            _diag_event("worker_finish", elapsed_ms=(time.time() - worker_t0) * 1000.0, payload={"status": "success", "project_name": args.project_name})
        else:
        ...
        _diag_event("worker_error", status="error", elapsed_ms=(time.time() - worker_t0) * 1000.0, payload={"error": str(e)})
