...
DEFAULT_CONFIG = {
    ...
    # 仿真
    "cpu_limit": get_default_cpu_limit(), "photons": 10000, "visibility_km": 3.0,
...
CSV_FIELD_LABELS = {
    "range_bins_m": "距离门中心 R (m)",
    "echo_I": "Stokes I 回波分量",
    "echo_Q": "Stokes Q 回波分量",
    "echo_U": "Stokes U 回波分量",
    "echo_V": "Stokes V 回波分量",
    "echo_power": "回波功率 P(R)",
    "echo_depol": "偏振损失指标",
    "echo_event_count": "回波采样事件数",
    "echo_weight_sum": "回波权重一阶矩",
    "echo_weight_sq_sum": "回波权重二阶矩",
    "echo_power_variance_est": "回波功率方差估计",
    "echo_power_ci_low": "回波功率 95% 置信区间下限",
    "echo_power_ci_high": "回波功率 95% 置信区间上限",
    "echo_relative_error_est": "回波相对误差估计",
    "echo_parallel_power": "平行偏振回波功率",
    "echo_perpendicular_power": "垂直偏振回波功率",
    "linear_depol_ratio": "线退偏比",
}

OPTIONAL_2D_CHART_OPTIONS = {
    "stokes": "Stokes I/Q/U/V",
    "normalized_stokes": "归一化 Stokes q/u/v",
    "polarization_channels": "平行/垂直偏振通道",
    "legacy_depol": "旧偏振损失指标",
    "event_count": "采样事件数",
    "relative_error": "相对误差",
    "variance": "回波方差估计",
                ...
                with ui.column().classes('w-full p-2 gap-1'):
                    ...
                    for key, value in sub_data.items():
                        ...
                        if key in SELECT_FIELD_OPTIONS:
                            ...
                            number_format = '%.6e' if key == 'm_imag' else '%.4g'
                            ui.number(label=label_text, value=value, format=number_format) \
                                .props('outlined dense').classes('w-full bg-white') \
                                .on_value_change(handler)
...
def _render_iitm_cpu_control():
    ...
    with ui.column().classes('w-full gap-1 mb-1'):
        ...
        fixed_input = ui.number(
            label='固定线程数',
            value=state.config_data.get('cpu_limit', get_default_cpu_limit()),
        ...
        def on_auto_toggle(e):
            ...
            if e.value:
                ...
                state.config_data['cpu_limit'] = int(fixed_input.value or get_default_cpu_limit())
    ...
    try:
        ...
        with zipfile.ZipFile(npz_path, "r") as zf:
            ...
            for key in LIDAR_DIAGNOSTIC_KEYS:
                ...
                if member in names:
                    arrays[key] = _read_npy_1d(zf.read(member))
        required = {"range_bins_m", "echo_power"}
        if not required.issubset(arrays):
            return {"available": False, "reason": "missing_default_echo_arrays"}
        if "linear_depol_ratio" not in arrays and "echo_depol" not in arrays:
            return {"available": False, "reason": "missing_polarization_summary"}
        ...
        return {
            "available": True,
            "arrays": arrays,
            "uses_legacy_depol": "linear_depol_ratio" not in arrays and "echo_depol" in arrays,
...
def _export_echo_observation_csv(include_header: bool = True) -> None:
    diag = _load_lidar_diagnostics()
    if not diag.get("available"):
        ui.notify(f"无法导出: {diag.get('reason', 'missing data')}", type="negative")
        return
    arrays = diag["arrays"]
    if "range_bins_m" not in arrays:
        ui.notify("无法导出: 缺少距离门数组", type="negative")
        return
    y_keys = [key for key in LIDAR_DIAGNOSTIC_KEYS if key != "range_bins_m" and key in arrays]
    if not y_keys:
        ui.notify("无法导出: 未找到可导出的 y 序列", type="negative")
        return
    keys = ["range_bins_m", *y_keys]
    n = min(len(arrays[key]) for key in keys)
    filename = "echo_observation_labeled.csv" if include_header else "echo_observation_data.csv"
    export_path = os.path.join(_current_export_dir(), filename)
    encoding = "utf-8-sig" if include_header else "utf-8"
    with open(export_path, "w", encoding=encoding, newline="") as f:
        writer = csv.writer(f)
        if include_header:
            writer.writerow([CSV_FIELD_LABELS.get(key, key) for key in keys])
        for i in range(n):
...
def _export_result_metadata_json() -> None:
    ...
    payload = {
        ...
        "observation_field_definitions": {
            "range_bins_m": "距离门中心 R，单位 m",
            "echo_power": "回波功率 P(R)，当前等同于 Stokes I 回波分量",
            ...
            "echo_depol": "旧偏振损失诊断字段，1 - sqrt(Q^2 + U^2 + V^2) / I；不作为默认线退偏比解释",
            "echo_event_count": "距离门内回波采样事件数",
            "echo_relative_error_est": "回波相对误差估计",
        },
...
def _render_export_menu(disabled: bool = False) -> None:
    ...
    with button:
        with ui.menu():
            ui.menu_item("导出观测 CSV（中文表头）", on_click=lambda: _export_echo_observation_csv(True))
            ui.menu_item("导出观测 CSV（纯数据）", on_click=lambda: _export_echo_observation_csv(False))
...
def _render_2d_diagnostics_page() -> None:
    ...
    with ui.column().classes("absolute inset-0 pt-16 p-4 gap-3 bg-gray-50 overflow-auto"):
        ...
        depol_key = "linear_depol_ratio" if "linear_depol_ratio" in arrays else "echo_depol"
        depol = arrays[depol_key]
        depol_label = "线退偏比 δ(R)" if depol_key == "linear_depol_ratio" else "旧偏振损失指标"
        with ui.card().classes("w-full p-3 gap-3").style("border-radius:8px;"):
            with ui.row().classes("w-full items-center gap-4"):
                ...
                ui.label(f"质量门限: count >= {state.echo_quality_min_events:g}, rel.err <= {state.echo_quality_max_rel_error:g}") \
                    .classes("text-xs text-gray-600")
                if diag.get("uses_legacy_depol"):
                    ui.label("当前结果缺少 linear_depol_ratio，已回退显示旧偏振损失指标。") \
        ...
        _series_chart(
            depol_label,
            ranges,
            [{"name": "linear depol" if depol_key == "linear_depol_ratio" else "legacy depol", "values": depol, "color": "#dc2626"}],
            y_name="depol",
            y_min=0,
            y_max=1,
        )
        ...
        if "polarization_channels" in selected_keys and all(k in arrays for k in ("echo_parallel_power", "echo_perpendicular_power")):
            _series_chart(
                "平行/垂直偏振回波通道",
                ranges,
                [
                    {"name": "parallel", "values": arrays["echo_parallel_power"], "color": "#2563eb"},
                    {"name": "perpendicular", "values": arrays["echo_perpendicular_power"], "color": "#dc2626"},
                ],
                y_name="power",
            )
        if "legacy_depol" in selected_keys and "echo_depol" in arrays:
            _series_chart(
                "旧偏振损失指标",
                ranges,
                [{"name": "echo_depol", "values": arrays["echo_depol"], "color": "#a16207"}],
                y_name="legacy depol",
                y_min=0,
                y_max=1,
            )
...
def _render_lidar_echo_panel() -> None:
    ...
    with ui.element("div").classes("absolute bottom-4 left-4 z-50").style(panel_style):
        ...
        depol_key = "linear_depol_ratio" if "linear_depol_ratio" in arrays else "echo_depol"
        depol_data = _downsample_xy(range_bins, arrays[depol_key])
        depol_label = "linear depol" if depol_key == "linear_depol_ratio" else "legacy depol"
...
async def run_simulation_script():
    ...
    raw_cpu = state.config_data.get('cpu_limit', get_default_cpu_limit())
...
def open_load_action(name: str):
    try:
        ...
        state.current_iitm_field = "beta_back"
        # 同步 iitm_cpu_auto 状态
        state.iitm_cpu_auto = (state.config_data.get('cpu_limit', get_default_cpu_limit()) == -1)
