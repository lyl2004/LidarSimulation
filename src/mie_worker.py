...
_DIAG_SESSION = get_or_create_session("mie_worker")
_DIAG_LOGGER = new_component_logger(_DIAG_SESSION, "mie_worker")
...
def save_field_npz(data, output_dir: Path):
    ...
    if "lidar_observation" in data:
        ...
        arrays.update({
            ...
            "echo_parallel_power": np.asarray(lidar["echo_parallel_power"], dtype=np.float32),
            "echo_perpendicular_power": np.asarray(lidar["echo_perpendicular_power"], dtype=np.float32),
            "linear_depol_ratio": np.asarray(lidar["linear_depol_ratio"], dtype=np.float32),
...
def main():
    ...
    parser.add_argument("--cpu_limit", type=str, default="auto")
    ...
    resolved_cpu_limit = get_default_cpu_limit() if args.cpu_limit == "auto" else max(1, int(args.cpu_limit))
    os.environ["NUMBA_NUM_THREADS"] = str(resolved_cpu_limit)
    os.environ["OMP_NUM_THREADS"] = str(resolved_cpu_limit)
    log_file = setup_logging(args.project_name)

    log_msg("="*60)
    log_msg(f">> [Init] MIE Worker Started (Offline/Embedded Mode)")
    log_msg(f">> [Info] Cores: {resolved_cpu_limit}")
    log_msg(f">> [Info] Log: {log_file}")
    log_msg("="*60)

    result_payload = {"status": "failed", "metrics": {}, "artifacts": []}
    worker_t0 = time.time()
    _diag_event("worker_start", status="begin", payload={"project_name": args.project_name, "cpu_limit": resolved_cpu_limit})
    ...
    try:
        ...
        _diag_event("worker_error", status="error", elapsed_ms=(time.time() - worker_t0) * 1000.0, payload={"error": str(e), "project_name": args.project_name})
