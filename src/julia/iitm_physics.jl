...
mutable struct MCLidarObservation
    ...
    echo_parallel_power :: Vector{Float64}
    echo_perpendicular_power :: Vector{Float64}
    linear_depol_ratio :: Vector{Float64}
...
function finalize_lidar_observation!(obs::MCLidarObservation, n_photons::Int)
    ...
    @inbounds for i in eachindex(obs.echo_I)
        ...
        obs.echo_parallel_power[i] = max(0.5 * (obs.echo_I[i] + obs.echo_Q[i]), 0.0)
        obs.echo_perpendicular_power[i] = max(0.5 * (obs.echo_I[i] - obs.echo_Q[i]), 0.0)
        if obs.echo_parallel_power[i] > 1e-30
            obs.linear_depol_ratio[i] = obs.echo_perpendicular_power[i] / obs.echo_parallel_power[i]
        else
            obs.linear_depol_ratio[i] = 0.0
        end
...
function save_field_npz(field::Dict, fields::Dict, scatter::Dict, output_dir::String)
    ...
    if haskey(bundle, "lidar_observation")
        ...
        arrays["echo_parallel_power"] = Float32.(obs.echo_parallel_power)
        arrays["echo_perpendicular_power"] = Float32.(obs.echo_perpendicular_power)
        arrays["linear_depol_ratio"] = Float32.(obs.linear_depol_ratio)
