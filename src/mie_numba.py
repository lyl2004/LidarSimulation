...
def run_advanced_simulation(
    ...
    if lidar_enabled:
        ...
        echo_parallel_power = np.maximum(0.5 * (echo_I_n + echo_Q_n), 0.0)
        echo_perpendicular_power = np.maximum(0.5 * (echo_I_n - echo_Q_n), 0.0)
        linear_depol_ratio = np.zeros_like(echo_I_n)
        parallel_mask = echo_parallel_power > 1e-30
        linear_depol_ratio[parallel_mask] = (
            echo_perpendicular_power[parallel_mask] / echo_parallel_power[parallel_mask]
        )
        ...
        arrays["lidar_observation"] = {
            ...
            "echo_parallel_power": echo_parallel_power,
            "echo_perpendicular_power": echo_perpendicular_power,
            "linear_depol_ratio": linear_depol_ratio,
